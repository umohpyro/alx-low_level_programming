...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	{
		new_node->next = current->next; /*current node after new node*/
		current->next = new_node;/*set the new node at index idx*/
	}
	else /*if node position is not present in the list*/
	{
		new_node->next = NULL;/*set next to NULL indicates last node*/
		current->next = new_node;/*set new node to last position*/
	}
